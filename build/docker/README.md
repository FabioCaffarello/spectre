...
proto generation: `core/control-plane`,
`adapters/curl-impersonate`, `adapters/playwright`, and
`adapters/seleniumbase`. The fifth (`core/engine`) does not —
Rust bindings are generated by `prost-build` inside the Cargo
build (`core/engine/build.rs`), not via `buf generate`.
...
crate (used in `core/engine/build.rs`) generates Rust types
...
stays out, and `core/engine/Dockerfile` is unchanged by R6.5.4.
