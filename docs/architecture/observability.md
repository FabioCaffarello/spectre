...
| W3.2 | Adapter OTel integration (Playwright / SeleniumBase / curl-impersonate): per-language SDK init (`@opentelemetry/sdk-node` + Pino / `opentelemetry-sdk` + structlog / `go.opentelemetry.io/otel` + slog), `:9090/metrics` sidecar (PrometheusExporter / `prometheus_client.start_http_server` / `promhttp`), `traceparent` extraction (HttpInstrumentation / `opentelemetry-instrumentation-grpc` / `otelgrpc.NewServerHandler`), per-RPC server-kind spans, §5.3 metric series, JSON stdout logs with the 11 mandatory §3.4 fields. Chart: three adapter Services gain `metrics` named port + observabilityEnv helper; ServiceMonitor selector extends to five components. Production-smoke gate asserts adapter `/metrics` plus a same-trace_id check across operator + engine + adapter logs (§4.2 topology end-to-end). | ✅ landed |
