"""Live ``Close`` conformance tests.

Each test launches the Playwright adapter, performs an
``Initialize`` to obtain a session id, then exercises the
``Close`` RPC. ``Close`` tears down the per-session
``BrowserContext`` and ``Page``; the shared ``Browser`` keeps
running so other sessions are unaffected. See ADR-0010.

Scenarios
---------

- ``test_close_happy_path``: Initialize → Navigate → Close
  succeeds and returns no ``DriverError``.
- ``test_close_then_navigate_fails``: a session that has been
  closed is rejected by subsequent ``Navigate`` calls with
  ``CODE_INVALID_ARGUMENT``.
- ``test_close_unknown_session_id_returns_invalid_argument``:
...
@pytest.mark.timeout(PER_TEST_DEADLINE_S)
def test_close_then_navigate_returns_invalid_argument(
    playwright_adapter: DriverHarness,
    local_http_server: LocalHttpServer,
) -> None:
    stub = driver_pb2_grpc.DriverStub(playwright_adapter.dial())
...
@pytest.mark.timeout(PER_TEST_DEADLINE_S)
def test_close_unknown_session_id_returns_invalid_argument(
    playwright_adapter: DriverHarness,
) -> None:
    stub = driver_pb2_grpc.DriverStub(playwright_adapter.dial())
