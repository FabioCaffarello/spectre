"""Live ``Query`` conformance tests for the SeleniumBase adapter.
...
Attribute) and the zero-matches contract. ADR-0010 §4 / §5
fixes the contract for both drivers; ADR-0015 §3 records the
SeleniumBase-specific TEXT-via-XPath mapping.

Scenarios
---------

- ``test_seleniumbase_query_css_returns_matches``: ``li.item``
  returns three ``ElementRef``s.
- ``test_seleniumbase_query_xpath_returns_matches``:
  ``//li[@class='item']`` returns three ``ElementRef``s.
- ``test_seleniumbase_query_text_returns_matches``: a substring
  text query returns at least one ``ElementRef``.
- ``test_seleniumbase_query_attribute_returns_matches``:
  ``data-test=primary`` returns one ``ElementRef``.
- ``test_seleniumbase_query_zero_matches_returns_empty_list``: a
  selector that does not match anything returns an empty
  ``elements`` list and no ``DriverError``.
"""
...
def test_seleniumbase_query_css_returns_matches(
...
@pytest.mark.timeout(PER_TEST_DEADLINE_S)
def test_seleniumbase_query_xpath_returns_matches(
    seleniumbase_adapter: DriverHarness,
    local_http_server: LocalHttpServer,
) -> None:
    stub = driver_pb2_grpc.DriverStub(seleniumbase_adapter.dial())
...
@pytest.mark.timeout(PER_TEST_DEADLINE_S)
def test_seleniumbase_query_text_returns_matches(
    seleniumbase_adapter: DriverHarness,
    local_http_server: LocalHttpServer,
) -> None:
    """SELECTOR_KIND_TEXT maps to an XPath ``contains(text(), …)``
    expression. ADR-0015 §3 documents the escape function; this
    test exercises the no-quote happy path."""
...
@pytest.mark.timeout(PER_TEST_DEADLINE_S)
def test_seleniumbase_query_attribute_returns_matches(
    seleniumbase_adapter: DriverHarness,
    local_http_server: LocalHttpServer,
) -> None:
    stub = driver_pb2_grpc.DriverStub(seleniumbase_adapter.dial())
...
@pytest.mark.timeout(PER_TEST_DEADLINE_S)
def test_seleniumbase_query_zero_matches_returns_empty_list(
    seleniumbase_adapter: DriverHarness,
    local_http_server: LocalHttpServer,
) -> None:
    """Zero matches is success with an empty list, not a DriverError.
    ADR-0010 §4 — the contract is the same on both drivers."""
